package com.ksc.network.slb.model;

import com.ksc.KscWebServiceRequest;
import com.ksc.Request;
import com.ksc.model.DryRunSupportedRequest;
import com.ksc.network.slb.model.transform.AssociateLoadBalancerAclRequestMarshaller;
import lombok.Data;

import java.io.Serializable;

/**
 * <p>
 * Contains the parameters for AssociateLoadBalancerAcl.
 * </p>
 */
@Data
public class AssociateLoadBalancerAclRequest extends KscWebServiceRequest implements
        Serializable, Cloneable, DryRunSupportedRequest<AssociateLoadBalancerAclRequest> {

    /**
     * 监听器的ID
     */
    private String listenerId;
    /**
     * 负载均衡ACL的ID
     */
    private String loadBalancerAclId;

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;
        if (!super.equals(o)) return false;

        AssociateLoadBalancerAclRequest that = (AssociateLoadBalancerAclRequest) o;
        if (listenerId != null ? !listenerId.equals(that.listenerId) : that.listenerId != null)
            return false;
        if (loadBalancerAclId != null ? !loadBalancerAclId.equals(that.loadBalancerAclId) : that.loadBalancerAclId != null)
            return false;
        return true;
    }

    @Override
    public int hashCode() {
        int result = super.hashCode();
        result = 31 * result + (listenerId != null ? listenerId.hashCode() : 0);
        result = 31 * result + (loadBalancerAclId != null ? loadBalancerAclId.hashCode() : 0);
        return result;
    }


    @Override
    public Request<AssociateLoadBalancerAclRequest> getDryRunRequest() {
        Request<AssociateLoadBalancerAclRequest> request = new AssociateLoadBalancerAclRequestMarshaller()
                .marshall(this);
        request.addParameter("DryRun", Boolean.toString(true));
        return request;
    }

    @Override
    public AssociateLoadBalancerAclRequest clone() {
        return (AssociateLoadBalancerAclRequest) super.clone();
    }
}

