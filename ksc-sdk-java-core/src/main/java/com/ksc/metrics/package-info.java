...
/**
 * Classes used to support the KSC SDK metrics API.
 * 
 * When the default metric collection system is enabled, 
 * the default KSC SDK implementation
 * captures a set of predefined core metrics that are grouped under three major 
 * categories:  KSC Request Metrics, KSC Service Metrics, and Machine Metrics.
 * <h3>Nomenclature</h3>
 * <ul>
 * <li>Physical Http Request - an http request initiated by the http client library used by KSC Java SDK . No retry is involved, or at least not initiated from the JVM.</li>
 * <li>Logical Http Request - an http request initiated from the KSC Java SDK by executing the underlying http client library. A logical request may involve multiple physical requests due to retries.</li>
 ...
 * round trip to KSC, and the time taken to execute the response handlers.
 ...
 * round trip to KSC. Captured on a per request type level.</li>
 * <li>HttpClientSendRequestTime - Number of milliseconds taken for a physical request
 * to get sent to KSC. Captured on a per request type level.</li>
 * <li>HttpClientReceiveResponseTime - Number of milliseconds taken for a physical response
 * to get received from KSC. Captured on a per request type level.</li>
