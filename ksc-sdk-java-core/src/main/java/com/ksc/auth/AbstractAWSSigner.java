...
import com.ksc.util.BinaryUtils;
...
public abstract class AbstractAWSSigner implements Signer {
     ...
     */
    public byte[] hash(String text) throws KscClientException {
        return AbstractAWSSigner.doHash(text);
    }

    private static byte[] doHash(String text) throws KscClientException {
        try {
    ...
    protected byte[] hash(InputStream input) throws KscClientException {
        try {
            MessageDigest md = MessageDigest.getInstance("SHA-256");
     ...
     *             If the hash cannot be computed.
     */
    public byte[] hash(byte[] data) throws KscClientException {
        try {
            MessageDigest md = MessageDigest.getInstance("SHA-256");
            md.update(data);
            return md.digest();
    ...
    protected byte[] getBinaryRequestPayloadWithoutQueryParams(SignableRequest<?> request) {
        ...
        try {
            ...
            throw new KscClientException("Unable to read request payload to sign request: " + e.getMessage(), e);
        }
    }
    ...
    protected InputStream getBinaryRequestPayloadStreamWithoutQueryParams(SignableRequest<?> request) {
        try {
            ...
            if (is == null)
                return new ByteArrayInputStream(new byte[0]);
            if (!is.markSupported())
                throw new KscClientException("Unable to read request payload to sign request.");
            return is;
