...
public class PolicyTest {
	...
	public void testPrincipals() {
		...
		policy.withStatements(new Statement(Effect.Allow)
				.withResources(new Resource("resource"))
				.withPrincipals(
						new Principal(Services.AllServices), 
						new Principal(Services.AllServices))
				.withActions(new TestAction("action")));
		...
		assertEquals(Services.AllServices.getServiceId(), services.get(0)
				.asText());
		assertEquals(Services.AllServices.getServiceId(), services.get(1)
				.asText());
		...
		policy.withStatements(new Statement(Effect.Allow)
				.withResources(new Resource("resource"))
				.withPrincipals(new Principal(Services.AllServices),
		...
		assertEquals(services.get(0).asText(),
				Services.AllServices.getServiceId());
	...
	public void testMultipleConditionKeysForConditionType() throws Exception {
		...
		JsonNode stringLikeCondition = conditions
				.get(StringComparisonType.StringNotLike.toString());
	...
	public void testPrincipalAccountId() {
		...
		Assert.assertEquals(ID_WITHOUT_HYPHEN, new Principal("AWS",
				ID_WITH_HYPHEN).getId());

		Assert.assertEquals(ID_WITH_HYPHEN, new Principal("Federated",
				ID_WITH_HYPHEN).getId());
		Assert.assertEquals(ID_WITH_HYPHEN, new Principal("AWS",
				ID_WITH_HYPHEN, false).getId());
	}
}
