...
import com.ksc.auth.policy.Principal.Services;
import com.ksc.auth.policy.Statement.Effect;
import com.ksc.auth.policy.conditions.ConditionFactory;
import com.ksc.auth.policy.conditions.IpAddressCondition;
import com.ksc.auth.policy.conditions.StringCondition;
import com.ksc.auth.policy.conditions.IpAddressCondition.IpAddressComparisonType;
import com.ksc.auth.policy.conditions.StringCondition.StringComparisonType;
...
public class PolicyReaderTest {
    ...
    public void testPrincipals() {
        ...
        policy = new Policy();
        policy.withStatements(new Statement(Effect.Allow).withResources(new Resource("resource")).withPrincipals(new Principal(Services.AllServices), new Principal(Services.AllServices))
                .withActions(new TestAction("action")));
        policy = Policy.fromJson(policy.toJson());
        assertEquals(1, policy.getStatements().size());
        ...
        assertEquals(Services.AllServices.getServiceId(), statements.get(0).getPrincipals().get(0).getId());
        assertEquals("Service", statements.get(0).getPrincipals().get(1).getProvider());
        assertEquals(Services.AllServices.getServiceId(), statements.get(0).getPrincipals().get(1).getId());
    ...
    public void testMultipleConditionKeysForConditionType() throws Exception {
        Policy policy = new Policy();
        policy.withStatements(new Statement(Effect.Allow)
          .withResources(new Resource("foo"))
    ...
    public void testFederatedUserBySAMLProvider() {
            ...
            "{" +
                ...
                "\"Statement\":[" +
                   "{" +
                      ...
                      "\"Condition\":{" +
                         "\"StringEquals\":{" +
                            "\"SAML:aud\":\"https://iam.api.ksyun.com\"" +
                         "}" +
                      "}" +
                   "}" +
                "]" +
        ...
        assertEquals(1, statements.get(0).getConditions().get(0).getValues().size());
        assertEquals("https://iam.api.ksyun.com", statements.get(0).getConditions().get(0).getValues().get(0));
        assertEquals(1, statements.get(0).getPrincipals().size());
        assertEquals("foo", statements.get(0).getPrincipals().get(0).getId());
    ...
    public void testPrincipalWithServiceNotInServicesEnum() {
        String jsonString = "{" + "\"Version\":\"2012-10-17\","
                + "\"Statement\":[" + "{" + "\"Sid\":\"\","
                + "\"Effect\":\"Allow\"," + "\"Principal\":{"
                + "\"Service\":\"kec.ksyun.com\" " + "},"
                + "\"Action\":\"sts:AssumeRole\"" + "}" + "]" + "}";
        ...
        assertEquals(0, statements.get(0).getConditions().size());
        assertEquals(1, statements.get(0).getPrincipals().size());
        assertEquals("kec.ksyun.com", statements.get(0)
